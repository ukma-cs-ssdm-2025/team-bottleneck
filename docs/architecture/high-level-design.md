...
## 1. Загальний опис
Наша система побудована за принципом **клієнт-серверної архітектури** з використанням REST API.  
Вона складається з фронтенду (React UI), бекенду (Python/Django) та бази даних (PostgreSQL).  
Система також інтегрується із зовнішніми сервісами (платіжна система, Email/SMS-провайдер, карти).
...
- **External Systems**: платіжна система, Email/SMS, Maps API.
...
- Обрано **React** для фронтенду, бо він популярний, має велику спільноту і легко інтегрується з REST API.  
(Деталі див. у ADR-001, ADR-002, ADR-003 у папці `docs/architecture/adr/`).

## 6. Технологічний стек
- **Frontend:** React, TypeScript  
- **Backend:** Python, Django (або Flask)  
- **Database:** PostgreSQL  
