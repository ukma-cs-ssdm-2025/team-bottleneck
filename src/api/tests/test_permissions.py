...
def test_has_permission_for_non_operator():
    user = User.objects.create_user(username="no_op", password=TEST_PASSWORD)
    request = factory.get("/")
    request.user = user
    permission = IsLotOperator()
...
def test_object_permission_different_lot_denied():
    user = User.objects.create_user(username="op2", password=TEST_PASSWORD)
    lot1 = ParkingLot.objects.create(name="Lot1", city="Kyiv", street="Main")
...
def test_operator_cannot_cancel_already_cancelled_booking():
    operator_user = User.objects.create_user(username="op_cancel_check", password=TEST_PASSWORD)
...
def test_operator_cannot_cancel_past_booking():
    operator_user = User.objects.create_user(username="op_past_cancel", password=TEST_PASSWORD)
